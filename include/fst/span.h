...
#if __FST_CPP_20__ || __FST_HAS_STD_SPAN__
#include <span>
...
//#define TCB_SPAN_NAMESPACE_NAME fst
...
// Reimplementation of C++17 std::size() and std::data()
  ...
  struct has_size_and_data<T,
      void_t<decltype(detail::size(std::declval<T>())), decltype(detail::data(std::declval<T>()))>> : std::true_type {};
  ...
  struct is_container_element_type_compatible<T, E,
      typename std::enable_if<
          !std::is_same<typename std::remove_cv<decltype(detail::data(std::declval<T>()))>::type, void>::value>::type>
...
public:
  ...
  using iterator = fst::wrap_iterator<pointer>;
  ...
  template <std::size_t N, std::size_t E = Extent,
      typename std::enable_if<(E == dynamic_extent || N == E)
              && detail::is_container_element_type_compatible<std::array<value_type, N>&, ElementType>::value,
          int>::type
      = 0>
  ...
  template <std::size_t N, std::size_t E = Extent,
      typename std::enable_if<(E == dynamic_extent || N == E)
              && detail::is_container_element_type_compatible<const std::array<value_type, N>&, ElementType>::value,
          int>::type
      = 0>
  ...
  using subspan_return_t = span<ElementType,
...
class tuple_size<fst::span<ElementType, Extent>> : public integral_constant<size_t, Extent> {};

template <typename ElementType>
class tuple_size<fst::span<ElementType, fst::dynamic_extent>>; // not defined

template <size_t I, typename ElementType, size_t Extent>
class tuple_element<I, fst::span<ElementType, Extent>> {
public:
  static_assert(Extent != fst::dynamic_extent && I < Extent, "");
...
// template <typename T>
// inline typename fst::span<T>::iterator begin(const fst::span<T>& s) {
//  return s.begin();
//}
//
// template <typename T>
// inline typename fst::span<T>::iterator begin(fst::span<T>& s) {
//  return s.begin();
//}
//
// template <typename T>
// inline typename fst::span<T>::iterator end(const fst::span<T>& s) {
//  return s.end();
//}
//
// template <typename T>
// inline typename fst::span<T>::iterator end(fst::span<T>& s) {
//  return s.end();
//}
//
// template <typename T, typename Tsize>
// inline fst::span<T> end(T* val, Tsize size) {
//  return fst::span<T>(val) + size;
//}
